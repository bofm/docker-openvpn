...
* Clone this repo and build the image
        ...
        git clone https://github.com/bofm/docker-openvpn.git
        cd docker-openvpn
        docker-build -t vpn .
...
* Initialize the `$OVPN_DATA` container that will hold the configuration files
  ...
  `VPN.SERVERNAME.COM` with IP address or public hostname of your server.
        ...
        docker run -v $OVPN_DATA:/etc/openvpn --rm -it vpn ovpn_initpki

* Start OpenVPN server processes (UDP + TCP)

        docker run --name vpn-udp -v $OVPN_DATA:/etc/openvpn -d -p 1194:1194/udp --cap-add=NET_ADMIN vpn
        docker run --name vpn-tcp -v $OVPN_DATA:/etc/openvpn --rm -p 443:1194/tcp --privileged vpn ovpn_run --proto tcp

* Generate a client certificate without a passphrase

        docker run -v $OVPN_DATA:/etc/openvpn --rm -it vpn easyrsa build-client-full CLIENTNAME nopass

* Retrieve the client configuration with embedded certificates

        docker run -v $OVPN_DATA:/etc/openvpn --rm vpn ovpn_getclient CLIENTNAME > CLIENTNAME.ovpn
